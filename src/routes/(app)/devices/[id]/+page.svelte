<script lang="ts">
	...
	import type { Device, EventLog, EventType } from '$lib/types';
		...
		try {
			...
			if (data.success) {
				...
				device = {
					...data.data,
					lastSeen:
						typeof data.data.lastSeen === 'string'
	...
	async function fetchDeviceEvents() {
		try {
			...
			if (data.success) {
				...
				events = data.data.map((event: ApiEventResponse) => ({
					...event,
					timestamp:
			...
			{#snippet children()}
				<div class="flex items-start justify-between">
					<div class="flex items-center gap-4">
						<div
							class="rounded-full p-3 {currentDevice.isOnline
						...
						>
							<svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
									...
									stroke-linejoin="round"
									stroke-width="2"
									d={deviceIcons[currentDevice.type] ?? deviceIcons.camera}
								/>
							</svg>
						</div>
						<div>
							<h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{currentDevice.name}</h1>
							<p class="text-sm text-zinc-500 capitalize dark:text-zinc-400">
								{currentDevice.type}
							</p>
						</div>
					</div>
				...
				<div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
					...
					<div>
						...
						<p class="mt-1 text-sm text-zinc-900 dark:text-white">
							{formatLastSeen(currentDevice.lastSeen)}
						</p>
					</div>
					{#if currentDevice.batteryLevel !== undefined}
						<div>
							...
							<div class="mt-1 flex items-center gap-2">
								<svg
									class="h-5 w-5 {currentDevice.batteryLevel > 20
										? 'text-green-500'
										: 'text-red-500'}"
									fill="currentColor"
									viewBox="0 0 24 24"
								...
								<span
									class="text-sm {currentDevice.batteryLevel > 20
										? 'text-zinc-900 dark:text-white'
										: 'text-red-500'}">{currentDevice.batteryLevel}%</span
								>
							</div>
						</div>
					{/if}
					...
					<div class="space-y-6">
						...
						<div class="flex items-start justify-between">
							<div class="flex-1">
								<div class="flex items-center gap-2">
									...
									<div class="relative">
										...
										{#if showTooltip}
											...
											>
												<div
													class="w-64 rounded-lg bg-zinc-900 px-3 py-2 text-xs text-white shadow-lg dark:bg-zinc-700"
												>
													Pre-event buffer continuously records video in memory, allowing the system
													to capture footage from before a motion event is triggered. This provides
													better context for security events.
													<div
														class="absolute top-full left-1/2 h-0 w-0 -translate-x-1/2 transform border-4 border-transparent border-t-zinc-900 dark:border-t-zinc-700"
													></div>
												</div>
											</div>
										{/if}
								...
								{#if preBufferEnabled}
									<div
										class="mt-2 rounded-md bg-amber-50 p-3 dark:bg-amber-900/20"
										data-testid="battery-impact-warning"
									>
										<div class="flex">
