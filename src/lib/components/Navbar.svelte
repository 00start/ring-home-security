...
<nav
	data-testid="navbar"
	class="border-b border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800"
>
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div class="flex h-16 justify-between">
			...
			<div class="flex items-center">
				{#if $user}
					<span class="mr-4 cursor-default text-sm text-zinc-500 dark:text-zinc-400">
						{$user.username}
					</span>
				{/if}
				<button
					...
					class="cursor-pointer rounded-md px-3 py-2 text-sm font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-700 dark:hover:text-zinc-300"
				>
					Logout
				</button>
			</div>
	...
	<!-- Mobile menu -->
	<div class="sm:hidden">
		<div class="flex space-x-1 overflow-x-auto px-2 pt-2 pb-3">
			{#each navItems as item}
				<a
					href={item.href}
