<script lang="ts">
	...
	const sensorSubtypeIcons: Record<string, string> = {
		contact:
			'M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z', // lock/door
		...
		smoke:
			'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' // alarm
	};

	// Subtype-specific icons for misc devices
	const miscSubtypeIcons: Record<string, string> = {
		base_station:
			'M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01', // server
		keypad:
			'M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z', // grid
		range_extender:
			'M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0', // wifi
		siren:
			'M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z' // speaker
	};
...
<Card>
	{#snippet children()}
		<button
			...
			class="-m-6 w-full rounded-lg p-6 text-left transition-transform hover:scale-[1.01] focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none"
		>
			<div class="flex items-start justify-between">
				<div class="flex items-center gap-3">
					<div
						class="rounded-full p-2 {device.isOnline
			...
			<div class="mt-4 flex items-center justify-between text-sm">
				...
				{#if device.batteryLevel !== undefined}
					<div class="flex items-center gap-1">
						<svg
							class="h-4 w-4 {device.batteryLevel > 20 ? 'text-green-500' : 'text-red-500'}"
							fill="currentColor"
							viewBox="0 0 24 24"
						>
							<path
						...
						<span
							class={device.batteryLevel > 20 ? 'text-zinc-600 dark:text-zinc-400' : 'text-red-500'}
		...
		{#if supportsLiveView(device.type) && device.isOnline}
			<div class="mt-3 border-t border-zinc-200 pt-3 dark:border-zinc-700">
				<button
					...
					type="button"
					class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700"
				>
					<svg
