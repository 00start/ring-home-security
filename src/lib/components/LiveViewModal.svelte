<script lang="ts">
	...
	function startNativePlayer() {
		...
		if (isIOSSafari) {
			error = 'Live streaming is not fully supported on iOS Safari. Please use a desktop browser for live view.';
			loading = false;
			return;
		}
					...
					{#if error}
						<div class="absolute inset-0 flex items-center justify-center">
							<div class="text-center p-6">
								...
								<p data-testid="error-message" data-error-type="live-stream" class="mt-4 text-sm text-red-500">{error}</p>
								{#if isIOSSafari}
									<p class="mt-2 text-xs text-zinc-400">iOS Safari doesn't support MPEG-TS streaming. For live view, please use Chrome or Firefox on a desktop computer.</p>
								{/if}
							</div>
						</div>
					{:else}
