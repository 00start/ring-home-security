...
for (const test of allFailedTests) {
	...
	if (
		...
		combinedError.includes('Test timeout') ||
		combinedError.includes('exceeded')
	) {
		category = 'TIMEOUT';
	} else if (
		combinedError.includes('element(s) not found') ||
